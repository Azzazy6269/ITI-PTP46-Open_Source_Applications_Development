...
const createPaymentSessionFromProvider = async (amount) => {
    ...
    const response = await httpClient.post('/v3/payment/sessions',{
       ...
       // merchantRedirect: "http://localhost:3000/redirect",///////
        merchantRedirect: "https://google.com",///////
        redirectMethod: null,
...
const updateDonationStatus =async (body)=>{
    ...
    if(!donation){
        throw new APIError("donation couldn't found",404);
    }
    const updateQuery = {
        $set: {
...
module.exports = { createPaymentSessionFromProvider , createDonation , handelWebhook , updateDonationStatus };